...
class StateDiagram {
    ...
    getStatePositions(mode) {
        const layouts = {
            batch: {
                IDLE: { x: 100, y: 100 },
                UPLOADING: { x: 250, y: 100 },
                WAITING_TRANSCRIBE: { x: 400, y: 100 },
                PROCESSING: { x: 550, y: 100 },
                ERROR: { x: 325, y: 250 },
                RECOVERING: { x: 325, y: 400 }
            },
            'non-streaming': {
                IDLE: { x: 100, y: 100 },
    ...
    getTransitions(mode) {
        const transitions = {
            batch: [
                { from: 'IDLE', to: 'UPLOADING', event: 'UPLOAD_FILE' },              // 使用者觸發上傳
                { from: 'UPLOADING', to: 'WAITING_TRANSCRIBE', event: 'UPLOAD_DONE' }, // 上傳完成
                { from: 'WAITING_TRANSCRIBE', to: 'PROCESSING', event: 'BEGIN_TRANSCRIPTION' }, // 正式進入轉譯
                { from: 'PROCESSING', to: 'IDLE', event: 'TRANSCRIPTION_DONE' },       // 轉譯結束，回到閒置

                // 錯誤處理
                { from: 'IDLE', to: 'ERROR', event: 'ERROR' },
                { from: 'UPLOADING', to: 'ERROR', event: 'ERROR' },
                { from: 'WAITING_TRANSCRIBE', to: 'ERROR', event: 'ERROR' },
