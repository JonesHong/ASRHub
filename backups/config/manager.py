...
try:
    ...
    SystemSchema,
    SessionsSchema,
    AudioQueueManagerSchema,
    LoggingSchema,
    ApiSchema,
    OperatorsSchema,
    AudioSchema,
    ProvidersSchema,
    WakeWordDetectionSchema,
    StreamSchema,
    FsmSchema,
    )
except ImportError:
...
class ConfigManager:
    ...
    @property
    def system(self) -> SystemSchema:
        """Get system configuration"""
        if not hasattr(self, '_system_cache'):
            self._system_cache = SystemSchema(self._data.get('system', {}))
    ...
    def sessions(self) -> SessionsSchema:
        ...
        if not hasattr(self, '_sessions_cache'):
            self._sessions_cache = SessionsSchema(self._data.get('sessions', {}))
        return self._sessions_cache

    @property
    def audio_queue_manager(self) -> AudioQueueManagerSchema:
        """Get audio_queue_manager configuration"""
        if not hasattr(self, '_audio_queue_manager_cache'):
            self._audio_queue_manager_cache = AudioQueueManagerSchema(self._data.get('audio_queue_manager', {}))
        return self._audio_queue_manager_cache
    ...
    def api(self) -> ApiSchema:
        ...
        if not hasattr(self, '_api_cache'):
            self._api_cache = ApiSchema(self._data.get('api', {}))
        return self._api_cache

    @property
    def operators(self) -> OperatorsSchema:
        """Get operators configuration"""
        if not hasattr(self, '_operators_cache'):
            self._operators_cache = OperatorsSchema(self._data.get('operators', {}))
        return self._operators_cache

    @property
    def audio(self) -> AudioSchema:
        """Get audio configuration"""
        if not hasattr(self, '_audio_cache'):
            self._audio_cache = AudioSchema(self._data.get('audio', {}))
        return self._audio_cache

    @property
    def providers(self) -> ProvidersSchema:
        """Get providers configuration"""
        if not hasattr(self, '_providers_cache'):
            self._providers_cache = ProvidersSchema(self._data.get('providers', {}))
        return self._providers_cache

    @property
    def wake_word_detection(self) -> WakeWordDetectionSchema:
        """Get wake_word_detection configuration"""
        if not hasattr(self, '_wake_word_detection_cache'):
            self._wake_word_detection_cache = WakeWordDetectionSchema(self._data.get('wake_word_detection', {}))
        return self._wake_word_detection_cache

    @property
    def stream(self) -> StreamSchema:
        """Get stream configuration"""
        if not hasattr(self, '_stream_cache'):
            self._stream_cache = StreamSchema(self._data.get('stream', {}))
        return self._stream_cache

    @property
    def fsm(self) -> FsmSchema:
        """Get fsm configuration"""
        if not hasattr(self, '_fsm_cache'):
            self._fsm_cache = FsmSchema(self._data.get('fsm', {}))
        return self._fsm_cache
