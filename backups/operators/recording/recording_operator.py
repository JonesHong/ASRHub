...
class RecordingOperator(OperatorBase):
    ...
    def __init__(self):
        ...
        try:
            ...
            recording_config = self.config_manager.operators.recording
    ...
    async def start_recording(self, session_id: str = None):
        ...
        if self.audio_queue_manager and session_id:
            try:
                # 獲取 pre-buffer 數據
                pre_recording = self.audio_queue_manager.get_pre_buffer_data(
                    session_id, 
                    seconds=self.pre_speech_buffer_duration
                )
                if pre_recording:
    ...
    async def get_recording_data(self, session_id: str, stop_if_recording: bool = True) -> Optional[bytes]:
        """
        獲取錄音數據（供 TranscriptionHandler 使用）
        
        Args:
            session_id: Session ID
            stop_if_recording: 如果正在錄音，是否停止錄音
        ...
        # 如果正在錄音且需要停止
        if self.is_recording and stop_if_recording:
        ...
        # 正在錄音但不停止，返回當前緩衝區的副本
        current_data = self.buffer.getvalue()
        logger.info(f"返回進行中的錄音數據: {len(current_data)} bytes")
        return current_data