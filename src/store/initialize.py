...
from src.core.audio_queue_manager import configure_audio_queue_manager
...
def create_operator_factories() -> Dict[str, Callable]:
    ...
    # Format Conversion
    def create_format_conversion(target_format="pcm", sample_rate=16000, channels=1):
        from src.operators.audio_format import ScipyAudioFormatOperator
        ...
        return ScipyAudioFormatOperator(
            operator_id='format_conversion',
    ...
    factories['format_conversion'] = create_format_conversion
...
async def initialize_asr_hub_store(
    ...
    audio_queue_manager = configure_audio_queue_manager(max_sessions=max_sessions)
    ...
    logger.info(f"  - {len(operator_factories)} Operator factories")
    logger.info(f"  - {len(provider_factories)} Provider factories")
