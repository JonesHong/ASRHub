...
class SessionEffects:
    ...
    def _trigger_transition(self, session_id: str, action: str) -> bool:
        ...
        try:
            ...
            old_state = fsm.state
            logger.info(f"🔄 FSM Transition: [{session_id}] Triggering '{action}' from state '{old_state}'")
            ...
            new_state = fsm.state
            if result:
                logger.info(f"✅ FSM State Changed: [{session_id}] '{old_state}' → '{new_state}' (action: {action})")
            else:
                logger.warning(f"❌ FSM Transition Failed: [{session_id}] Attempted '{action}' from '{old_state}', staying at '{new_state}'")
            return result
    ...
    def _is_processing(self, session_id: str) -> bool:
        ...
        return fsm.is_processing(allow_substates=True) if hasattr(fsm, 'is_processing') else fsm.state.startswith('processing')
    ...
    def _is_activated(self, session_id: str) -> bool:
        ...
        if not fsm:
            return False
        return fsm.is_processing_activated() if hasattr(fsm, 'is_processing_activated') else fsm.state == 'processing_activated'
    ...
    def _is_recording(self, session_id: str) -> bool:
        ...
        if not fsm:
            return False
        return fsm.is_processing_recording() if hasattr(fsm, 'is_processing_recording') else fsm.state == 'processing_recording'
    ...
    def _is_transcribing(self, session_id: str) -> bool:
        ...
        if not fsm:
            return False
        return fsm.is_processing_transcribing() if hasattr(fsm, 'is_processing_transcribing') else fsm.state == 'processing_transcribing'
    ...
    def _process_audio_chunk(self, action):
        ...
        if not audio_config:
            ...
            logger.error("Audio config must be set when session is created or via SET_AUDIO_METADATA")
            return
    ...
    def _start_wake_word_monitoring(self, session_id: str):
        ...
        success = openwakeword.start_listening(
            ...
            callback=lambda detection: self._on_wake_word_detected(
                session_id, 
                detection.keyword,  # 傳遞 keyword 作為 source
                detection.timestamp,
    ...
    def _on_wake_word_detected(self, session_id: str, source: str, timestamp: float, confidence: float):
        """處理喚醒詞檢測到事件 - 使用現有錄音服務"""
        logger.info(f"✅ Wake word detected: '{source}' (confidence: {confidence:.3f}) at {timestamp:.3f} for session {session_id}")
        ...
        self.store.dispatch(record_started({
            "session_id": session_id,
            "timestamp": recording_start,
            "metadata": recording_metadata
        }))
        
        logger.info(f"Recording started from {recording_start:.3f} (pre-roll {self.pre_roll_duration}s)")
        ...
        def vad_callback(result):
            ...
            if is_speech:
                ...
                if silence_start is not None:
                    logger.info(f"🎤 Speech detected (confidence: {confidence:.3f}), silence timer reset")
                    silence_start = None
                    # 停止靜音計時器
                ...
                fsm = self._get_or_create_fsm(session_id)
                is_recording = fsm and hasattr(fsm, 'is_processing_recording') and fsm.is_processing_recording()
                if silence_start is None and is_recording:
                    silence_start = current_time
                    logger.info(f"🤫 Silence started at {silence_start:.3f} (confidence: {confidence:.3f})")
                    # 派發靜音檢測 action（包含時間戳）
                    self.store.dispatch(vad_silence_detected({
                        "session_id": session_id,
    ...
    def _on_silence_timeout(self, session_id: str, timestamp: float):
        ...
        self.store.dispatch(record_stopped({
            "session_id": session_id,
    ...
    def _batch_process_audio(self, session_id: str, audio_chunks: List[TimestampedAudio], recording_filepath: Optional[str] = None):
        """批量處理錄音數據（降噪、增強、ASR）
        ...
        if recording_filepath:
            ...
            if os.path.exists(recording_filepath):
                ...
                logger.warning(f"Recording file not found: {recording_filepath}, falling back to audio chunks")
        ...
        if config.services.denoiser.enabled and HAS_DEEPFILTERNET:
            ...
            if config.services.denoiser.enabled and not HAS_DEEPFILTERNET:
                logger.warning("DeepFilterNet not available (PyTorch not installed), skipping denoising")
            denoised_audio = combined_audio
        ...
        if config.services.audio_enhancer.enabled:
            ...
            try:
                enhanced_audio, report = audio_enhancer.auto_enhance(
                    denoised_audio,
                    purpose="asr"  # 正確的參數名稱是 purpose 而非 preset
                )
    ...
    def handle_session_expired(self, action_stream):
        ...
        return action_stream.pipe(
            ...
            ops.do_action(self._cleanup_session)
        )
    ...
    def handle_reset_session(self, action_stream):
        """處理重置會話"""
        return action_stream.pipe(
            ofType(reset_session),
            ops.do_action(self._reset_session)
        )
    ...
    def _reset_session(self, action):
        ...
        if session_id in self._fsm_instances:
            ...
            if fsm:
                ...
                logger.info(f"🔄 FSM Reset: [{session_id}] Recreated FSM, initial state: {fsm.state}")
                old_state = fsm.state
                success = False
                if hasattr(fsm, 'start_listening'):
                ...
                if success:
                    ...
                    logger.warning(f"Failed to transition FSM to listening state for session {session_id}")
        else:
            # 如果沒有 FSM 實例，創建一個新的
            fsm = self._get_or_create_fsm(session_id)
            if fsm:
    ...
    def handle_start_listening(self, action_stream):
        ...
        return action_stream.pipe(
            ...
            ops.do_action(self._init_session_listening)
        )
    ...
    def handle_wake_activated(self, action_stream):
        """處理喚醒詞激活"""
        return action_stream.pipe(
            ofType(wake_activated),
            ops.do_action(self._on_wake_activated)
        )
    ...
    def handle_error_occurred(self, action_stream):
        """處理錯誤發生"""
        return action_stream.pipe(
            ofType(error_occurred),
            ops.do_action(self._on_error_occurred)
        )
    ...
    def handle_wake_deactivated(self, action_stream):
        """處理喚醒詞停用"""
        return action_stream.pipe(
            ofType(wake_deactivated),
            ops.do_action(self._on_wake_deactivated)
        )
    ...
    def handle_upload_started(self, action_stream):
        ...
        return action_stream.pipe(
            ...
            ops.do_action(self._on_upload_started)
        )
    ...
    def handle_upload_completed(self, action_stream):
        """處理檔案上傳完成"""
        return action_stream.pipe(
            ofType(upload_completed),
            ops.do_action(self._on_upload_completed)
        )
    ...
    def handle_asr_stream_started(self, action_stream):
        """處理 ASR 串流開始"""
        return action_stream.pipe(
            ofType(asr_stream_started),
            ops.do_action(self._on_asr_stream_started)
        )
    ...
    def handle_asr_stream_stopped(self, action_stream):
        """處理 ASR 串流停止"""
        return action_stream.pipe(
            ofType(asr_stream_stopped),
            ops.do_action(self._on_asr_stream_stopped)
        )
