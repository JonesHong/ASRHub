...
class ProviderPool:
        ...
        try:
            ...
            try:
                wrapper = await asyncio.wait_for(
                    self._acquire_provider(),
                    timeout=self.acquire_timeout
                )
            except asyncio.TimeoutError:
                self.stats.failed_requests += 1
                raise TimeoutError(
                    f"獲取 {self.provider_type} 超時 ({self.acquire_timeout}秒)"
                )
    ...
    async def health_check(self) -> Dict[str, Any]:
        ...
        if self.stats.current_size < self.min_size:
            health_info["issues"].append(f"池大小低於最小值 ({self.stats.current_size} < {self.min_size})")
    ...
    def get_pool_status_table(self) -> Table:
        ...
        # 添加行
        table.add_row("Pool Size", f"{self.stats.current_size}/{self.max_size}", "✓" if self.stats.current_size >= self.min_size else "⚠")
                ...
                if hasattr(provider, 'transcribe'):
                    ...
                    try:
                        await asyncio.wait_for(
                            provider.transcribe(test_audio_data),
                            timeout=5.0
                        )
                        # 成功，重置錯誤計數
                        wrapper.reset_error()
        ...
        while not self._available.empty():
            try:
                ...
                if (wrapper.last_used_at and 
                    now - wrapper.last_used_at > idle_timeout_delta and
    ...
    async def scale(self, new_size: int):
        ...
        if new_size < self.min_size or new_size > self.max_size:
            raise ValueError(
                f"新大小必須在 {self.min_size} 和 {self.max_size} 之間"
            )
    ...
    def log_metrics(self):
        ...
        self.logger.table(
            [
                ...
                ["成功率", f"{metrics['successful_requests'] / max(metrics['total_requests'], 1):.2%}"],
